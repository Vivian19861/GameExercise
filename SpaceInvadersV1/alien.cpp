...
Alien::Alien(int x, int y)
    : velocity{2.0}, shotVelocity{3}, shotFired{false}, isDestroyed{false} {
  shape.x = x;
  shape.y = y;
  shape.w = alienWidth;
  shape.h = alienHeight;
...
void Alien::moveRight() { shape.x += velocity; }

void Alien::moveLeft() { shape.x -= velocity; }

void Alien::moveDown() { shape.y += 5; }
...
SDL_Rect Alien::getShape() { return shape; }
