...
float cal_angle(float y1, float y2, int height){
...
void Init(SDL_Window** win, SDL_Renderer** ren){
  if( SDL_Init(SDL_INIT_EVERYTHING) < 0 ){
    ...
    *win = SDL_CreateWindow("Pong Game by Wei",
...
int main(int argc, char* argv[]){
  ...
  while(!quit){
    ...
    */
    ...
    renderTexture(squareTex, gRenderer, SCREEN_WIDTH/2 - CENTER_WIDTH/2, 0, CENTER_WIDTH, SCREEN_HEIGHT);
    renderTexture(squareTex, gRenderer, b.x, b.y, BALL_WIDTH, BALL_HEIGHT);

    sprintf(buffer, "%d", p1.score);
    SDL_Texture *p1score = renderText(buffer, "./fonts/sample.ttf", whiteColor, 40, gRenderer);
    sprintf(buffer, "%d", p2.score);
    SDL_Texture *p2score = renderText(buffer, "./fonts/sample.ttf", whiteColor, 40, gRenderer);
    ...
    SDL_Texture *fpsCounter = renderText(buffer, "./fonts/sample.ttf", whiteColor, 20, gRenderer);
